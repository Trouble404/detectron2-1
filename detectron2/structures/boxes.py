...
def intersection_cacl(
    boxes1: Boxes, boxes2: Boxes
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    Given two lists of boxes of size N and M,
    compute their independent boxes areas and intersection areas.
    Args:
        boxes1,boxes2 (Boxes): two `Boxes`. Contains N & M boxes, respectively.

    Returns:
        Tensor: intersection, sized [N,M].
        Tensor: area1, area2, sized N and M
    """
    area1 = boxes1.area()  # [N]
    area2 = boxes2.area()  # [M]
...
def pairwise_iou(boxes1: Boxes, boxes2: Boxes) -> torch.Tensor:
    ...
    """
    inter, area1, area2 = intersection_cacl(boxes1, boxes2)
...
def pairwise_ioa(boxes1: Boxes, boxes2: Boxes) -> torch.Tensor:
    """
    Similar to pariwise_iou but compute the IoA (intersection over boxes1 area).
    ...
    # handle empty boxes
    ioa = torch.where(
        inter > 0, inter / area1[:, None], torch.zeros(1, dtype=inter.dtype, device=inter.device)
    )
    return ioa
